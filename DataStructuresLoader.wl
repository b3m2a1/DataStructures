...
(* ::Text::GrayLevel[0.5]:: *)
(* Autogenerated DataStructures loader file *)


Temp`PackageScope`DataStructuresLoading`Private`$DependencyLoad=
  TrueQ@Temp`PackageScope`DataStructuresLoading`Private`$DependencyLoad;
If[Temp`PackageScope`DataStructuresLoading`Private`$DependencyLoad,
  Unprotect["`DataStructures`PackageScope`Private`Constants`$TopLevelLoad"];
  Evaluate[Symbol["`DataStructures`PackageScope`Private`Constants`$TopLevelLoad"]]=False,
  Unprotect["DataStructures`PackageScope`Private`Constants`$TopLevelLoad"];
  Evaluate[Symbol["DataStructures`PackageScope`Private`Constants`$TopLevelLoad"]]=
    MemberQ[$ContextPath, "Global`"]
  ];


If[Temp`PackageScope`DataStructuresLoading`Private`$DependencyLoad,
  Evaluate[Symbol["`DataStructures`PackageScope`Private`$RootContext"]]=$Context;
  BeginPackage["`DataStructures`"],
  Evaluate[Symbol["DataStructures`PackageScope`Private`$RootContext"]]="";
  BeginPackage["DataStructures`"]
  ]


ClearAll[DataStructures];
DataStructures::usage="DataStructures is a manager head for the DataStructures package";
...
DataStructures["LoadingParameters"]:=$PackageLoadSpecs
$PackageLoadSpecs=
  Merge[
    {
      With[
...
DataStructures["Mode"]:=$PackageLoadingMode
$PackageLoadingMode=
...
DataStructures["Dependencies"]:=$PackageDependencies;
$PackageDependencies=
...
DataStructures["DependencyBase"]:=$PackageDependencyBase;
...
DataStructures["PackageListing"]:=$PackageListing;
$PackageListing=<||>;
DataStructures["Contexts"]:=$PackageContexts;
  ...
  $PackageContexts=
    Join[
      If[$PackageLoadingMode==="Dependency",
        ...
        {
          "DataStructures`",
          "DataStructures`PackageScope`Private`",
          "DataStructures`PackageScope`Package`"
          },
...
DataStructures["FEScopedSymbols"]:=$PackageFEHiddenSymbols;
$PackageFEHiddenSymbols={};
DataStructures["PackageScopedSymbols"]:=$PackageScopedSymbols;
...
DataStructures["FileContexts"]:=$PackageFileContexts;
...
DataStructures["DeclaredPackages"]:=$DeclaredPackages;
...
DataStructures["LoadedPackages"]:=$LoadedPackages;
...
PackageFileContext[f_String?DirectoryQ]:=
  With[{s=PackageFileContextPath[f]},
    StringRiffle[Append[""]@Prepend[s,DataStructures],"`"]
    ];
...
DataStructures::nodep="Couldn't load dependency `` of type ``";
DataStructures::nodup="Couldn't update dependency `` of type ``";
  ...
  Block[{retcode, site, path, file, tmp},
    ...
    site=
      ...
      If[retcode<300,
        ...
        DeleteFile[tmp];
        file,
        Message[DataStructures::nodep, dep, "Package"];
        DeleteFile[tmp];
        $Failed
        ]
      ];
    ...
    Monitor[
      MapThread[
        Check[
          ...
          Message[DataStructures::nodep, #2, "Paclet"];
          $Failed
          ]&,
        {
          pacs,
      ...
      Monitor[
        MapThread[
          Check[
            ...
            Message[DataStructures::nodup, #2, "Paclet"];
            $Failed
            ]&,
          {
            StringDelete[deps, "`"],
...
$PackageErrorMessage=
  "DataStructures encountered exception ``";
...
PackageThrowMessage[
  ...
  PackageThrowMessage[
    MessageName[DataStructures, tag],
    body,
    ops
    ];
PackageThrowMessage[
...
PackageCatchMessage[
  ...
  Replace[
    Thread[Map[Hold[MessageName[DataStructures, #]]&, msg], Hold],
    {
      {}:>
        Check[expr, 
...
PackageFailureException[
  ...
  (
    Set[MessageName[DataStructures, tag], body];
    PackageFailureException[
      MessageName[DataStructures, tag],
      body,
      ops
...
PackageAttachMainAutocomplete[]:=
  PackageAddAutocompletions[DataStructures, 
    Table[
      Replace[{}->None]@
        Cases[
          DownValues[DataStructures],
          Nest[
            Insert[#, _, {1, 1, 1}]&,
            (HoldPattern[Verbatim[HoldPattern]][
              DataStructures[s_String, ___]
              ]:>_),
            n-1
            ]:>s,
          Infinity
...
PackagePrepPackageSymbol[]:=
  Switch[$AllowPackageSymbolDefinitions,
    None,
      If[Length@OwnValues[DataStructures]==0,
...
If[Temp`PackageScope`DataStructuresLoading`Private`$DependencyLoad,
  If[
    (Clear@`DataStructures`PackageScope`Private`$loadAbort;!#)&@
      `DataStructures`PackageScope`Private`$loadAbort,
    Unprotect[`DataStructures`PackageScope`Private`$LoadCompleted];
    `DataStructures`PackageScope`Private`$LoadCompleted=True;
    `DataStructures`PackageScope`Private`PackagePostProcessContextPathReassign[]
    ],
  If[
    (Clear@DataStructures`PackageScope`Private`$loadAbort;!#)&@
      DataStructures`PackageScope`Private`$loadAbort,
...
Remove["Temp`PackageScope`DataStructuresLoading`Private`*"]